...
def structure_check_action(biobb_block: PluginBlock):
    ...
    output_summary_path_value = biobb_block.inputs["output_summary_path"]
    
    # Define the properties for the biobb tool
    properties_values = {}
    
    properties_values["features"] = biobb_block.variables["features"]
    ...
    with open("structure_check.json", "w", encoding="utf-8") as f:
    ...
    with subprocess.Popen(
        [
            ...
            "--output_summary_path",
            f"/tmp/{Path(output_summary_path_value).name}",
        ...
        abs_path_out = Path(output_summary_path_value).absolute()
        abs_path_name = Path(Path(output_summary_path_value).name).absolute()
        if not abs_path_name.samefile(abs_path_out):
            shutil.copy(f"{Path(output_summary_path_value).name}", output_summary_path_value)
        biobb_block.setOutput("output_summary_path", output_summary_path_value)
...
outputs_list.append(output_summary_path)


variables_list.append(features)
...
structure_check_block = PluginBlock(
