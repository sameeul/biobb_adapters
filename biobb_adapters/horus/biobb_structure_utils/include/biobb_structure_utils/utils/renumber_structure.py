...
output_mapping_json_path = PluginVariable(
    id="output_mapping_json_path",  # ID of the variable, will allow us to identify the value
    name="output_mapping_json_path",  # The name that will appear in the frontend
    description="Output mapping json file path",  # The description that will appear in the frontend
    type=VariableTypes.FILE,  # The type. This will render the variable accrodingly
    # The allowedValues parameter depends on the type of variable,
    # in the case of files, they denote the allowed extensions.
    allowedValues=['json']
...
renumber_residues = PluginVariable(
    id="renumber_residues",
    name="renumber_residues",
    description="Residue code of the ligand to be removed.",
    type=VariableTypes.BOOLEAN
)

renumber_residues_per_chain = PluginVariable(
    id="renumber_residues_per_chain",
    name="renumber_residues_per_chain",
...
def renumber_structure_action(biobb_block: PluginBlock):
    ...
    output_mapping_json_path_value = biobb_block.inputs["output_mapping_json_path"]
    
    # Define the properties for the biobb tool
    properties_values = {}
    
    properties_values["renumber_residues"] = biobb_block.variables["renumber_residues"]
    
    properties_values["renumber_residues_per_chain"] = biobb_block.variables["renumber_residues_per_chain"]
    ...
    with open("renumber_structure.json", "w", encoding="utf-8") as f:
    ...
    with subprocess.Popen(
        [
            ...
            f"/tmp/{Path(output_mapping_json_path_value).name}",
        ...
        # Copy the outputs to the output folder
        abs_path_out = Path(output_mapping_json_path_value).absolute()
        abs_path_name = Path(Path(output_mapping_json_path_value).name).absolute()
        if not abs_path_name.samefile(abs_path_out):
            shutil.copy(f"{Path(output_mapping_json_path_value).name}", output_mapping_json_path_value)
        biobb_block.setOutput("output_mapping_json_path", output_mapping_json_path_value)
...
outputs_list.append(output_mapping_json_path)


variables_list.append(renumber_residues)

variables_list.append(renumber_residues_per_chain)
...
renumber_structure_block = PluginBlock(
    # The name which will appear on the frontend
    name="renumber_structure",
    # Its description
    description="Class to renumber atomic indexes from a 3D structure.",
    # The action
    action=renumber_structure_action,
