...
def pdb_cluster_zip_action(biobb_block: PluginBlock):
    ...
    output_pdb_zip_path_value = biobb_block.inputs["output_pdb_zip_path"]
    ...
    properties_values["filter"] = biobb_block.variables["filter"]
    
    properties_values["cluster"] = biobb_block.variables["cluster"]
    ...
    with open("pdb_cluster_zip.json", "w", encoding="utf-8") as f:
    ...
    with subprocess.Popen(
        [
            ...
            "--output_pdb_zip_path",
            f"/tmp/{Path(output_pdb_zip_path_value).name}",
        ...
        abs_path_out = Path(output_pdb_zip_path_value).absolute()
        abs_path_name = Path(Path(output_pdb_zip_path_value).name).absolute()
        if not abs_path_name.samefile(abs_path_out):
            shutil.copy(f"{Path(output_pdb_zip_path_value).name}", output_pdb_zip_path_value)
        biobb_block.setOutput("output_pdb_zip_path", output_pdb_zip_path_value)
...
outputs_list.append(output_pdb_zip_path)


variables_list.append(pdb_code)

variables_list.append(filter)

variables_list.append(cluster)
...
pdb_cluster_zip_block = PluginBlock(
