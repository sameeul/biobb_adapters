...
def memprotmd_sim_action(biobb_block: PluginBlock):
    ...
    output_simulation_value = biobb_block.inputs["output_simulation"]
    ...
    with open("memprotmd_sim.json", "w", encoding="utf-8") as f:
    ...
    with subprocess.Popen(
        [
            ...
            "memprotmd_sim",
            "--config",
            "/tmp/memprotmd_sim.json",
            
            
            "--output_simulation",
            f"/tmp/{Path(output_simulation_value).name}",
        ...
        abs_path_out = Path(output_simulation_value).absolute()
        abs_path_name = Path(Path(output_simulation_value).name).absolute()
        if not abs_path_name.samefile(abs_path_out):
            shutil.copy(f"{Path(output_simulation_value).name}", output_simulation_value)
        biobb_block.setOutput("output_simulation", output_simulation_value)
...
outputs_list.append(output_simulation)
...
memprotmd_sim_block = PluginBlock(
