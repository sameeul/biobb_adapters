...
def canonical_fasta_action(biobb_block: PluginBlock):
    ...
    output_fasta_path_value = biobb_block.inputs["output_fasta_path"]
    ...
    with open("canonical_fasta.json", "w", encoding="utf-8") as f:
    ...
    with subprocess.Popen(
        [
            ...
            "--output_fasta_path",
            f"/tmp/{Path(output_fasta_path_value).name}",
        ...
        abs_path_out = Path(output_fasta_path_value).absolute()
        abs_path_name = Path(Path(output_fasta_path_value).name).absolute()
        if not abs_path_name.samefile(abs_path_out):
            shutil.copy(f"{Path(output_fasta_path_value).name}", output_fasta_path_value)
        biobb_block.setOutput("output_fasta_path", output_fasta_path_value)
...
outputs_list.append(output_fasta_path)
...
canonical_fasta_block = PluginBlock(
