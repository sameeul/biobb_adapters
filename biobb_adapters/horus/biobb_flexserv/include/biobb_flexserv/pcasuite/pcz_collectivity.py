...
output_json_path = PluginVariable(
    ...
    description="Output json file with PCA Collectivity indexes per mode",  # The description that will appear in the frontend
    type=VariableTypes.FILE,  # The type. This will render the variable accrodingly
    # The allowedValues parameter depends on the type of variable,
    # in the case of files, they denote the allowed extensions.
    allowedValues=['json']
...
eigenvector = PluginVariable(
    id="eigenvector",
    name="eigenvector",
    description="PCA mode (eigenvector) from which to extract stiffness.",
...
def pcz_collectivity_action(biobb_block: PluginBlock):
    ...
    with open("pcz_collectivity.json", "w", encoding="utf-8") as f:
...
pcz_collectivity_block = PluginBlock(
    # The name which will appear on the frontend
    name="pcz_collectivity",
    # Its description
    description="Extract PCA collectivity (numerical measure of how many atoms are affected by a given mode) from a compressed PCZ file.",
    # The action
    action=pcz_collectivity_action,
