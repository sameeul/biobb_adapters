...
output_json_path = PluginVariable(
    ...
    description='Output json file with PCA Stiffness',  # The description that will appear in the frontend
    type=VariableTypes.FILE,  # The type. This will render the variable accrodingly
    # The allowedValues parameter depends on the type of variable,
    # in the case of files, they denote the allowed extensions.
    allowedValues=['json']
...
temperature = PluginVariable(
    id="temperature",
    name="temperature",
    description='Temperature with which compute the apparent stiffness.',
    type=VariableTypes.INTEGER
)
...
def pcz_stiffness_action(biobb_block: PluginBlock):
    ...
    with open("pcz_stiffness.json", "w", encoding="utf-8") as f:
...
pcz_stiffness_block = PluginBlock(
    # The name which will appear on the frontend
    name="pcz_stiffness",
    # Its description
    description='Extract PCA stiffness from a compressed PCZ file.',
    # The action
    action=pcz_stiffness_action,
