...
output_str_path = PluginVariable(
    id="output_str_path",  # ID of the variable, will allow us to identify the value
    name="output_str_path",  # The name that will appear in the frontend
    description='Path to the output file',  # The description that will appear in the frontend
    type=VariableTypes.FILE,  # The type. This will render the variable accrodingly
    # The allowedValues parameter depends on the type of variable,
    # in the case of files, they denote the allowed extensions.
    allowedValues=['pdb', 'xtc', 'trr', 'cpt', 'gro', 'g96', 'tng']
)
...
def gmx_trjconv_str_action(biobb_block: PluginBlock):
    ...
    output_str_path_value = biobb_block.inputs["output_str_path"]
    ...
    with open("gmx_trjconv_str.json", "w", encoding="utf-8") as f:
    ...
    with subprocess.Popen(
        [
            ...
            "--output_str_path",
            f"/tmp/{Path(output_str_path_value).name}",
        ...
        abs_path_out = Path(output_str_path_value).absolute()
        abs_path_name = Path(Path(output_str_path_value).name).absolute()
        if not abs_path_name.samefile(abs_path_out):
            shutil.copy(f"{Path(output_str_path_value).name}", output_str_path_value)
        biobb_block.setOutput("output_str_path", output_str_path_value)
...
outputs_list.append(output_str_path)
...
gmx_trjconv_str_block = PluginBlock(
