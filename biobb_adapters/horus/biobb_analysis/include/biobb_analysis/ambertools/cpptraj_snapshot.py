...
output_cpptraj_path = PluginVariable(
    ...
    description='Path to the output processed structure',  # The description that will appear in the frontend
    type=VariableTypes.FILE,  # The type. This will render the variable accrodingly
    # The allowedValues parameter depends on the type of variable,
    # in the case of files, they denote the allowed extensions.
    allowedValues=['mdcrd', 'crd', 'netcdf', 'nc', 'rst7', 'ncrst', 'dcd', 'pdb', 'mol2', 'binpos', 'trr', 'xtc', 'sqm']
)
...
snapshot = PluginVariable(
    id="snapshot",
    name="snapshot",
    description='Frame to be captured for snapshot',
...
def cpptraj_snapshot_action(biobb_block: PluginBlock):
    ...
    properties_values["snapshot"] = biobb_block.variables["snapshot"]
    
    properties_values["mask"] = biobb_block.variables["mask"]
    
    properties_values["format"] = biobb_block.variables["format"]
    ...
    with open("cpptraj_snapshot.json", "w", encoding="utf-8") as f:
...
variables_list.append(snapshot)

variables_list.append(mask)

variables_list.append(format)
...
cpptraj_snapshot_block = PluginBlock(
    # The name which will appear on the frontend
    name="cpptraj_snapshot",
    # Its description
    description='Wrapper of the Ambertools Cpptraj module for extracting a particular snapshot from a given cpptraj compatible trajectory.',
    # The action
    action=cpptraj_snapshot_action,
