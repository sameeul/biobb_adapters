...
def puckering_action(biobb_block: PluginBlock):
    ...
    input_phaseC_path_value = biobb_block.inputs["input_phaseC_path"]
    
    input_phaseW_path_value = biobb_block.inputs["input_phaseW_path"]
    ...
    with open("puckering.json", "w", encoding="utf-8") as f:
    ...
    with subprocess.Popen(
        [
            ...
            "--input_phaseC_path",
            f"/tmp/{Path(input_phaseC_path_value).name}",
            
            "--input_phaseW_path",
            f"/tmp/{Path(input_phaseW_path_value).name}",
...
inputs_list.append(input_phaseC_path)

inputs_list.append(input_phaseW_path)
