...
input_cpout_path = PluginVariable(
    id="input_cpout_path",  # ID of the variable, will allow us to identify the value
    name="input_cpout_path",  # The name that will appear in the frontend
    description="Output constant pH file (AMBER cpout)",  # The description that will appear in the frontend
    type=VariableTypes.FILE,  # The type. This will render the variable accrodingly
    # The allowedValues parameter depends on the type of variable,
    # in the case of files, they denote the allowed extensions.
    allowedValues=['cpout', 'zip', 'gzip']
)
...
output_dat_path = PluginVariable(
    ...
    description="Output file to which the standard calcpka-type statistics are written",  # The description that will appear in the frontend
    type=VariableTypes.FILE,  # The type. This will render the variable accrodingly
    # The allowedValues parameter depends on the type of variable,
    # in the case of files, they denote the allowed extensions.
...
def cphstats_run_action(biobb_block: PluginBlock):
    ...
    input_cpout_path_value = biobb_block.inputs["input_cpout_path"]
    ...
    properties_values["protonated"] = biobb_block.variables["protonated"]
    
    properties_values["pka"] = biobb_block.variables["pka"]
    
    properties_values["calcpka"] = biobb_block.variables["calcpka"]
    ...
    with open("cphstats_run.json", "w", encoding="utf-8") as f:
    ...
    with subprocess.Popen(
        [
            ...
            "cphstats_run",
            "--config",
            "/tmp/cphstats_run.json",
            
            "--input_cpin_path",
            f"/tmp/{Path(input_cpin_path_value).name}",
            
            "--input_cpout_path",
            f"/tmp/{Path(input_cpout_path_value).name}",
...
inputs_list.append(input_cpout_path)
...
variables_list.append(protonated)

variables_list.append(pka)

variables_list.append(calcpka)
...
cphstats_run_block = PluginBlock(
    # The name which will appear on the frontend
    name="cphstats_run",
    # Its description
    description="Wrapper of the AmberTools (AMBER MD Package) cphstats tool module.",
    # The action
    action=cphstats_run_action,
