...
output_cpin_path = PluginVariable(
    id="output_cpin_path",  # ID of the variable, will allow us to identify the value
    name="output_cpin_path",  # The name that will appear in the frontend
    description='Output AMBER constant pH input (CPin) file',  # The description that will appear in the frontend
    type=VariableTypes.FILE,  # The type. This will render the variable accrodingly
    # The allowedValues parameter depends on the type of variable,
    # in the case of files, they denote the allowed extensions.
    allowedValues=['cpin']
...
def parmed_cpinutil_action(biobb_block: PluginBlock):
    ...
    output_cpin_path_value = biobb_block.inputs["output_cpin_path"]
    ...
    properties_values["resnames"] = biobb_block.variables["resnames"]
    
    properties_values["igb"] = biobb_block.variables["igb"]
    
    properties_values["system"] = biobb_block.variables["system"]
    ...
    with open("parmed_cpinutil.json", "w", encoding="utf-8") as f:
    ...
    with subprocess.Popen(
        [
            ...
            "--output_cpin_path",
            f"/tmp/{Path(output_cpin_path_value).name}",
        ...
        abs_path_out = Path(output_cpin_path_value).absolute()
        abs_path_name = Path(Path(output_cpin_path_value).name).absolute()
        if not abs_path_name.samefile(abs_path_out):
            shutil.copy(f"{Path(output_cpin_path_value).name}", output_cpin_path_value)
        biobb_block.setOutput("output_cpin_path", output_cpin_path_value)
...
outputs_list.append(output_cpin_path)

outputs_list.append(output_top_path)


variables_list.append(resnames)

variables_list.append(igb)

variables_list.append(system)
...
parmed_cpinutil_block = PluginBlock(
    # The name which will appear on the frontend
    name="parmed_cpinutil",
    # Its description
    description='Wrapper of the AmberTools (AMBER MD Package) parmed tool module.',
    # The action
    action=parmed_cpinutil_action,
