...
input_crd_path = PluginVariable(
    ...
    # The allowedValues parameter depends on the type of variable,
    # in the case of files, they denote the allowed extensions.
    allowedValues=['crd', 'mdcrd', 'inpcrd', 'rst', 'rst7', 'netcdf', 'nc', 'ncrst']
)
...
def pmemd_mdrun_action(biobb_block: PluginBlock):
    ...
    with open("pmemd_mdrun.json", "w", encoding="utf-8") as f:
...
pmemd_mdrun_block = PluginBlock(
    # The name which will appear on the frontend
    name="pmemd_mdrun",
    # Its description
    description='Wrapper of the AmberTools (AMBER MD Package) pmemd tool module.',
    # The action
    action=pmemd_mdrun_action,
