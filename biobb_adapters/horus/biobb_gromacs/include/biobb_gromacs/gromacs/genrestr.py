...
def genrestr_action(biobb_block: PluginBlock):
    ...
    output_itp_path_value = biobb_block.inputs["output_itp_path"]
    
    # Define the properties for the biobb tool
    properties_values = {}
    
    properties_values["restrained_group"] = biobb_block.variables["restrained_group"]
    
    properties_values["force_constants"] = biobb_block.variables["force_constants"]
    ...
    with open("genrestr.json", "w", encoding="utf-8") as f:
    ...
    with subprocess.Popen(
        [
            ...
            "--output_itp_path",
            f"/tmp/{Path(output_itp_path_value).name}",
        ...
        abs_path_out = Path(output_itp_path_value).absolute()
        abs_path_name = Path(Path(output_itp_path_value).name).absolute()
        if not abs_path_name.samefile(abs_path_out):
            shutil.copy(f"{Path(output_itp_path_value).name}", output_itp_path_value)
        biobb_block.setOutput("output_itp_path", output_itp_path_value)
...
outputs_list.append(output_itp_path)


variables_list.append(restrained_group)

variables_list.append(force_constants)
