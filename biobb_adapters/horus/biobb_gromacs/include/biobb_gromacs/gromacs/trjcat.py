...
output_trj_path = PluginVariable(
    ...
    # The allowedValues parameter depends on the type of variable,
    # in the case of files, they denote the allowed extensions.
    allowedValues=['pdb', 'gro', 'xtc', 'trr', 'tng']
)
...
def trjcat_action(biobb_block: PluginBlock):
    ...
    input_trj_zip_path_value = biobb_block.inputs["input_trj_zip_path"]
    
    
    output_trj_path_value = biobb_block.inputs["output_trj_path"]
    
    # Define the properties for the biobb tool
    properties_values = {}
    
    properties_values["concatenate"] = biobb_block.variables["concatenate"]
    ...
    with open("trjcat.json", "w", encoding="utf-8") as f:
    ...
    with subprocess.Popen(
        [
            ...
            "--input_trj_zip_path",
            f"/tmp/{Path(input_trj_zip_path_value).name}",
            
            
            "--output_trj_path",
            f"/tmp/{Path(output_trj_path_value).name}",
        ...
        abs_path_out = Path(output_trj_path_value).absolute()
        abs_path_name = Path(Path(output_trj_path_value).name).absolute()
        if not abs_path_name.samefile(abs_path_out):
            shutil.copy(f"{Path(output_trj_path_value).name}", output_trj_path_value)
        biobb_block.setOutput("output_trj_path", output_trj_path_value)
...
inputs_list.append(input_trj_zip_path)


outputs_list.append(output_trj_path)


variables_list.append(concatenate)
