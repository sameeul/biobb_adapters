...
output_path_gro = PluginVariable(
    id="output_path_gro",  # ID of the variable, will allow us to identify the value
    name="output_path_gro",  # The name that will appear in the frontend
    description='Path to the GRO output file',  # The description that will appear in the frontend
    type=VariableTypes.FILE,  # The type. This will render the variable accrodingly
    # The allowedValues parameter depends on the type of variable,
    # in the case of files, they denote the allowed extensions.
    allowedValues=['gro']
...
output_path_itp = PluginVariable(
    id="output_path_itp",  # ID of the variable, will allow us to identify the value
    name="output_path_itp",  # The name that will appear in the frontend
    description='Path to the ITP output file',  # The description that will appear in the frontend
    type=VariableTypes.FILE,  # The type. This will render the variable accrodingly
    # The allowedValues parameter depends on the type of variable,
    # in the case of files, they denote the allowed extensions.
    allowedValues=['itp']
...
def acpype_params_gmx_action(biobb_block: PluginBlock):
    ...
    output_path_gro_value = biobb_block.inputs["output_path_gro"]
    
    output_path_itp_value = biobb_block.inputs["output_path_itp"]
    ...
    with open("acpype_params_gmx.json", "w", encoding="utf-8") as f:
    ...
    with subprocess.Popen(
        [
            ...
            "--output_path_gro",
            f"/tmp/{Path(output_path_gro_value).name}",
            
            "--output_path_itp",
            f"/tmp/{Path(output_path_itp_value).name}",
        ...
        abs_path_out = Path(output_path_gro_value).absolute()
        abs_path_name = Path(Path(output_path_gro_value).name).absolute()
        if not abs_path_name.samefile(abs_path_out):
            shutil.copy(f"{Path(output_path_gro_value).name}", output_path_gro_value)
        biobb_block.setOutput("output_path_gro", output_path_gro_value)
        
        # Copy the outputs to the output folder
        abs_path_out = Path(output_path_itp_value).absolute()
        abs_path_name = Path(Path(output_path_itp_value).name).absolute()
        if not abs_path_name.samefile(abs_path_out):
            shutil.copy(f"{Path(output_path_itp_value).name}", output_path_itp_value)
        biobb_block.setOutput("output_path_itp", output_path_itp_value)
...
outputs_list.append(output_path_gro)

outputs_list.append(output_path_itp)
