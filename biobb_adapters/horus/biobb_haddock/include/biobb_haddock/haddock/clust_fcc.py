...
output_cluster_zip_path = PluginVariable(
    id="output_cluster_zip_path",  # ID of the variable, will allow us to identify the value
    name="output_cluster_zip_path",  # The name that will appear in the frontend
...
def clust_fcc_action(biobb_block: PluginBlock):
    ...
    output_cluster_zip_path_value = biobb_block.inputs["output_cluster_zip_path"]
    ...
    with open("clust_fcc.json", "w", encoding="utf-8") as f:
    ...
    with subprocess.Popen(
        [
            ...
            f"/tmp/{Path(output_cluster_zip_path_value).name}",
        ...
        abs_path_out = Path(output_cluster_zip_path_value).absolute()
        abs_path_name = Path(Path(output_cluster_zip_path_value).name).absolute()
        if not abs_path_name.samefile(abs_path_out):
            shutil.copy(f"{Path(output_cluster_zip_path_value).name}", output_cluster_zip_path_value)
        biobb_block.setOutput("output_cluster_zip_path", output_cluster_zip_path_value)
...
outputs_list.append(output_cluster_zip_path)
...
clust_fcc_block = PluginBlock(
    # The name which will appear on the frontend
    name="clust_fcc",
    # Its description
    description="Wrapper class for the Haddock ClustFCC module.",
    # The action
    action=clust_fcc_action,
