...
reference_pdb_path = PluginVariable(
    id="reference_pdb_path",  # ID of the variable, will allow us to identify the value
    name="reference_pdb_path",  # The name that will appear in the frontend
    description='Path to the input PDB file containing an structure for reference',  # The description that will appear in the frontend
    type=VariableTypes.FILE,  # The type. This will render the variable accrodingly
    # The allowedValues parameter depends on the type of variable,
    # in the case of files, they denote the allowed extensions.
    allowedValues=['pdb']
...
output_evaluation_zip_path = PluginVariable(
    id="output_evaluation_zip_path",  # ID of the variable, will allow us to identify the value
    name="output_evaluation_zip_path",  # The name that will appear in the frontend
...
def capri_eval_action(biobb_block: PluginBlock):
    ...
    reference_pdb_path_value = biobb_block.inputs["reference_pdb_path"]
    
    haddock_config_path_value = biobb_block.inputs["haddock_config_path"]
    
    
    output_evaluation_zip_path_value = biobb_block.inputs["output_evaluation_zip_path"]
    ...
    with open("capri_eval.json", "w", encoding="utf-8") as f:
    ...
    with subprocess.Popen(
        [
            ...
            "--reference_pdb_path",
            f"/tmp/{Path(reference_pdb_path_value).name}",
            ...
            f"/tmp/{Path(output_evaluation_zip_path_value).name}",
        ...
        abs_path_out = Path(output_evaluation_zip_path_value).absolute()
        abs_path_name = Path(Path(output_evaluation_zip_path_value).name).absolute()
        if not abs_path_name.samefile(abs_path_out):
            shutil.copy(f"{Path(output_evaluation_zip_path_value).name}", output_evaluation_zip_path_value)
        biobb_block.setOutput("output_evaluation_zip_path", output_evaluation_zip_path_value)
...
inputs_list.append(reference_pdb_path)

inputs_list.append(haddock_config_path)


outputs_list.append(output_evaluation_zip_path)
...
capri_eval_block = PluginBlock(
    # The name which will appear on the frontend
    name="capri_eval",
    # Its description
    description='Wrapper class for the Haddock CapriEval module.',
    # The action
    action=capri_eval_action,
