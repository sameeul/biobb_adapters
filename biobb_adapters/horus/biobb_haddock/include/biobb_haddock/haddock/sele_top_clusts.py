...
output_selection_zip_path = PluginVariable(
    id="output_selection_zip_path",  # ID of the variable, will allow us to identify the value
    name="output_selection_zip_path",  # The name that will appear in the frontend
...
def sele_top_clusts_action(biobb_block: PluginBlock):
    ...
    output_selection_zip_path_value = biobb_block.inputs["output_selection_zip_path"]
    ...
    with open("sele_top_clusts.json", "w", encoding="utf-8") as f:
    ...
    with subprocess.Popen(
        [
            ...
            f"/tmp/{Path(output_selection_zip_path_value).name}",
        ...
        abs_path_out = Path(output_selection_zip_path_value).absolute()
        abs_path_name = Path(Path(output_selection_zip_path_value).name).absolute()
        if not abs_path_name.samefile(abs_path_out):
            shutil.copy(f"{Path(output_selection_zip_path_value).name}", output_selection_zip_path_value)
        biobb_block.setOutput("output_selection_zip_path", output_selection_zip_path_value)
...
outputs_list.append(output_selection_zip_path)
...
sele_top_clusts_block = PluginBlock(
    # The name which will appear on the frontend
    name="sele_top_clusts",
    # Its description
    description='Wrapper class for the Haddock SeleTopClusts module https://www.bonvinlab.org/haddock3/modules/analysis/seletopclusts.html.',
    # The action
    action=sele_top_clusts_action,
