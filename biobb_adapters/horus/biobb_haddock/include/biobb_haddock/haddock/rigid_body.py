...
ambig_restraints_table_path = PluginVariable(
    id="ambig_restraints_table_path",  # ID of the variable, will allow us to identify the value
    name="ambig_restraints_table_path",  # The name that will appear in the frontend
    description="Path to the input TBL file containing a list of ambiguous restraints for docking",  # The description that will appear in the frontend
    type=VariableTypes.FILE,  # The type. This will render the variable accrodingly
    # The allowedValues parameter depends on the type of variable,
    # in the case of files, they denote the allowed extensions.
    allowedValues=['tbl']
...
unambig_restraints_table_path = PluginVariable(
    id="unambig_restraints_table_path",  # ID of the variable, will allow us to identify the value
    name="unambig_restraints_table_path",  # The name that will appear in the frontend
    description="Path to the input TBL file containing a list of unambiguous restraints for docking",  # The description that will appear in the frontend
    type=VariableTypes.FILE,  # The type. This will render the variable accrodingly
    # The allowedValues parameter depends on the type of variable,
    # in the case of files, they denote the allowed extensions.
    allowedValues=['tbl']
...
hb_restraints_table_path = PluginVariable(
    id="hb_restraints_table_path",  # ID of the variable, will allow us to identify the value
    name="hb_restraints_table_path",  # The name that will appear in the frontend
    description="Path to the input TBL file containing a list of hydrogen bond restraints for docking",  # The description that will appear in the frontend
    type=VariableTypes.FILE,  # The type. This will render the variable accrodingly
    # The allowedValues parameter depends on the type of variable,
    # in the case of files, they denote the allowed extensions.
    allowedValues=['tbl']
...
docking_output_zip_path = PluginVariable(
    id="docking_output_zip_path",  # ID of the variable, will allow us to identify the value
    name="docking_output_zip_path",  # The name that will appear in the frontend
...
def rigid_body_action(biobb_block: PluginBlock):
    ...
    ambig_restraints_table_path_value = biobb_block.inputs["ambig_restraints_table_path"]
    
    unambig_restraints_table_path_value = biobb_block.inputs["unambig_restraints_table_path"]
    
    hb_restraints_table_path_value = biobb_block.inputs["hb_restraints_table_path"]
    
    haddock_config_path_value = biobb_block.inputs["haddock_config_path"]
    
    
    docking_output_zip_path_value = biobb_block.inputs["docking_output_zip_path"]
    ...
    with open("rigid_body.json", "w", encoding="utf-8") as f:
    ...
    with subprocess.Popen(
        [
            ...
            f"/tmp/{Path(ambig_restraints_table_path_value).name}",
            
            "--unambig_restraints_table_path",
            f"/tmp/{Path(unambig_restraints_table_path_value).name}",
            
            "--hb_restraints_table_path",
            f"/tmp/{Path(hb_restraints_table_path_value).name}",
            ...
            f"/tmp/{Path(docking_output_zip_path_value).name}",
        ...
        abs_path_out = Path(docking_output_zip_path_value).absolute()
        abs_path_name = Path(Path(docking_output_zip_path_value).name).absolute()
        if not abs_path_name.samefile(abs_path_out):
            shutil.copy(f"{Path(docking_output_zip_path_value).name}", docking_output_zip_path_value)
        biobb_block.setOutput("docking_output_zip_path", docking_output_zip_path_value)
...
inputs_list.append(ambig_restraints_table_path)

inputs_list.append(unambig_restraints_table_path)

inputs_list.append(hb_restraints_table_path)

inputs_list.append(haddock_config_path)


outputs_list.append(docking_output_zip_path)
...
rigid_body_block = PluginBlock(
    # The name which will appear on the frontend
    name="rigid_body",
    # Its description
    description="Wrapper class for the Haddock RigidBody module.",
    # The action
    action=rigid_body_action,
