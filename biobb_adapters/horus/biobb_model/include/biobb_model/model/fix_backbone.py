...
input_fasta_canonical_sequence_path = PluginVariable(
    id="input_fasta_canonical_sequence_path",  # ID of the variable, will allow us to identify the value
    name="input_fasta_canonical_sequence_path",  # The name that will appear in the frontend
    description="Input FASTA file path",  # The description that will appear in the frontend
    type=VariableTypes.FILE,  # The type. This will render the variable accrodingly
    # The allowedValues parameter depends on the type of variable,
    # in the case of files, they denote the allowed extensions.
    allowedValues=['fasta']
...
def fix_backbone_action(biobb_block: PluginBlock):
    ...
    input_fasta_canonical_sequence_path_value = biobb_block.inputs["input_fasta_canonical_sequence_path"]
    ...
    properties_values["add_caps"] = biobb_block.variables["add_caps"]
    ...
    with open("fix_backbone.json", "w", encoding="utf-8") as f:
    ...
    with subprocess.Popen(
        [
            ...
            f"/tmp/{Path(input_fasta_canonical_sequence_path_value).name}",
...
inputs_list.append(input_fasta_canonical_sequence_path)


outputs_list.append(output_pdb_path)


variables_list.append(add_caps)
...
fix_backbone_block = PluginBlock(
    # The name which will appear on the frontend
    name="fix_backbone",
    # Its description
    description="Class to model the missing atoms in the backbone of a PDB structure.",
    # The action
    action=fix_backbone_action,
