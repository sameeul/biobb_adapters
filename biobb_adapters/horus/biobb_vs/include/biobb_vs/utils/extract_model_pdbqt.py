...
input_pdbqt_path = PluginVariable(
    id="input_pdbqt_path",  # ID of the variable, will allow us to identify the value
    name="input_pdbqt_path",  # The name that will appear in the frontend
    description="Input PDBQT file",  # The description that will appear in the frontend
...
output_pdbqt_path = PluginVariable(
    ...
    description="Output PDBQT file",  # The description that will appear in the frontend
    type=VariableTypes.FILE,  # The type. This will render the variable accrodingly
    # The allowedValues parameter depends on the type of variable,
    # in the case of files, they denote the allowed extensions.
    allowedValues=['pdbqt']
...
def extract_model_pdbqt_action(biobb_block: PluginBlock):
    ...
    input_pdbqt_path_value = biobb_block.inputs["input_pdbqt_path"]
    ...
    properties_values["model"] = biobb_block.variables["model"]
    ...
    with open("extract_model_pdbqt.json", "w", encoding="utf-8") as f:
    ...
    with subprocess.Popen(
        [
            ...
            "--input_pdbqt_path",
            f"/tmp/{Path(input_pdbqt_path_value).name}",
...
inputs_list.append(input_pdbqt_path)


outputs_list.append(output_pdbqt_path)


variables_list.append(model)
