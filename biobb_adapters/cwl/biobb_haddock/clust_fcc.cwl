...
label: Wrapper class for the Haddock ClustFCC module.

doc: |-
  The ClustFCC module. Haddock ClustFCC  module computes clusters of structures using FCC.

baseCommand: clust_fcc

hints:
  DockerRequirement:
    dockerPull: quay.io/biocontainers/biobb_haddock:4.1.0--pyhdfd78af_0
...
inputs:
  ...
  output_cluster_zip_path:
    ...
    doc: |-
      ...
      File type: output
      Accepted formats: zip
      Example file: https://raw.githubusercontent.com/bioexcel/biobb_haddock/master/biobb_haddock/test/reference/haddock/ref_clustfcc.zip
    type: string
    format:
    - edam:format_3987
    inputBinding:
      position: 2
      prefix: --output_cluster_zip_path
    default: system.zip
  ...
  config:
    label: Advanced configuration options for biobb_haddock ClustFCC
    doc: |-
      Advanced configuration options for biobb_haddock ClustFCC. This should be passed as a string containing a dict. The possible options to include here are listed under 'properties' in the biobb_haddock ClustFCC documentation: https://biobb-haddock.readthedocs.io/en/latest/haddock.html#module-haddock.clust_fcc
    type: string?
    inputBinding:
      prefix: --config
...
outputs:
  output_cluster_zip_path:
    ...
    type: File
    outputBinding:
      glob: $(inputs.output_cluster_zip_path)
