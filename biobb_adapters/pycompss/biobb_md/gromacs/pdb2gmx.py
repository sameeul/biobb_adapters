...
@task(input_pdb_path=FILE_IN, output_gro_path=FILE_OUT,
      output_top_zip_path=FILE_OUT,
      on_failure="IGNORE", time_out=task_time_out)
def _pdb2gmx(input_pdb_path, output_gro_path,
             output_top_zip_path,
             properties, **kwargs):
    task_config.pop_pmi(os.environ)
    try:
        Pdb2gmx(input_pdb_path=input_pdb_path, output_gro_path=output_gro_path,
...
def pdb2gmx(input_pdb_path, output_gro_path,
            output_top_zip_path,
            properties=None, **kwargs):
    if (input_pdb_path is None or os.path.exists(input_pdb_path)) and \
       ...
       (output_top_zip_path is None or os.path.exists(output_top_zip_path)):
        ...
        _pdb2gmx(input_pdb_path, output_gro_path,
                 output_top_zip_path,
                 properties, **kwargs)
